    ...
    describe('PUT /booking', () => {
        ...
        describe('when token is valid', () => {
            it('should respond with status 200 with a valid body', async () => {
                ...
                const diffRoom = await createRoomWithHotelId(hotel.id);
            ...
            it('should respond with status 403 when user does not have a booking', async () => {
                ...
                const diffUser = await createUser();
                const diffUserBooking = await createBooking({
                    roomId: room.id,
                    userId: diffUser.id
                });
                const validBody = createValidBody();

                const response = await server.put(`/booking/${diffUserBooking.id}`).set('Authorization', `Bearer ${token}`).send({
                roomId: room.id
                });
                expect(response.status).toEqual(httpStatus.FORBIDDEN);
            });
